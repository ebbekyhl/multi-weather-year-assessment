...
def calculate_summary(n,label,df):
    ...
    co2_cap = n.global_constraints.loc['CO2Limit'].constant
    df.loc['co2_cap', label] = round(co2_cap,sig_dig)

    load_shedders = n.generators.query('carrier == "load"')
    lost_load = n.generators_t.p[load_shedders].sum().sum()
    df.loc['lost_load', label] = round(lost_load,sig_dig)
    ...
    for ind in df_shares.index:
        df.loc[ind + '_share', label] = round(df_shares.loc[ind],sig_dig)
    ########################################################################
    ########################################################################
...
def make_summaries(networks_dict,design_network):
    ...
    for output in outputs:
        df[output] = globals()["calculate_" + output](n_design, 'design', df[output])
...
if __name__ == "__main__":
   ...
   + "/resolved_n"+ nodes + "_" + tres + f"h_dy{design_year}_wy{weather_year}.nc"
