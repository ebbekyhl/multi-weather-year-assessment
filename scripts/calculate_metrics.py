...
for dyear in dyears:
    ...
    for str_i in strings:
        ...
        capacity_deficit_c = {}
        capacity_deficit_c_pct = {}
        capacity_deficit_c_EU_hour = {}
        unserved_energy_c_avlh = {}
        unserved_energy_EU_TWh = {}
        unserved_energy_c_EU_hour = {}
        ...
        for j in range(no_networks):
            ...
            opts = network_names[j].split('/')[5].split('_')
            year = opts[4][2:] # weather year
            print(year)
            ...
            peak_capacity_deficits_EU = load_shedders_t.sum(axis=1).max()
            peak_capacity_deficits_EU_index = load_shedders_t.sum(axis=1).idxmax()

            peak_capacity_deficit = load_shedders_t_country.max() # % of peak load
            peak_capacity_deficit_pct = (load_shedders_t_country/peak_load*100).max() # % of peak load
            peak_capacity_deficit_c_EU_peak_hour = load_shedders_t_country.loc[peak_capacity_deficits_EU_index] # country peak capacity deficit in worst EU aggregate hour

            unserved_energy = load_shedders_t_country.sum()*weighting # MWh
            # 14 days rolling sum
            freq = int(24/3*14)
            unserved_energy_14_days_rolling_EU = load_shedders_t_country.rolling(freq).sum().sum(axis=1)
            unserved_energy_14_days_rolling_EU_index = unserved_energy_14_days_rolling_EU.idxmax()
            unserved_energy_14_days_rolling_c = load_shedders_t_country.rolling(freq).sum().loc[unserved_energy_14_days_rolling_EU_index]

            capacity_deficit_c[year] = peak_capacity_deficit/1e3 # GW
            capacity_deficit_c_EU_hour[year] = peak_capacity_deficit_c_EU_peak_hour/1e3 # GW
            capacity_deficit_c_pct[year] = peak_capacity_deficit_pct
            ...
            unserved_energy_c_EU_hour[year] = unserved_energy_14_days_rolling_c/1e3 # GWh
        ...
        variables_dict = {
                        ...
                        # country specific data
                            ...
                            "unserved_energy_rolling_14_days_EU_hour_country":unserved_energy_c_EU_hour,
                            "capacity_deficit_country":capacity_deficit_c,
                            "capacity_deficit_pct_country":capacity_deficit_c_pct,
                            "capacity_deficit_EU_hour_country":capacity_deficit_c_EU_hour,
                            }
